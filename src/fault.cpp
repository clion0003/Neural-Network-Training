...
namespace mlp{
	void Fault::generateVariation(double sigma)
	{
	...
	void Fault::generateLogVariation(double sigma)
	{
		assert(row != 0 && column != 0);
	...
	void Fault::generateSA0(double sa0_fault_ratio, int sa0_faulty_columns)
	{
		for (int i = 0; i < row; i++)
		{
			for (int j = 0; j < column; j++)
	...
	void Fault::generateSA1(double sa1_fault_ratio, int sa1_faulty_rows, int sa1_bad_block)
	{
		/*************************************   set sa1   *********************************************************/
		//set bad block
		boost::mt19937 gen(time(NULL));
	...
	void Fault::resizeFault(int row_size, int column_size)
	{
		column = column_size;
		row = row_size;
	...
	float Fault::getFaultValue(int row_index, int col_index)
	{
		assert(row_index < row&&col_index < column);
		return fault_array[row_index][col_index].value;
	}

	int	Fault::getFaultType(int row_index, int col_index)
	{
		assert(row_index < row&&col_index < column);
		return fault_array[row_index][col_index].type;
	}

	void Fault::setFaultValue(int row_index, int col_index, double v)
	{
		assert(row_index < row&&col_index < column);
		fault_array[row_index][col_index].value = v;
	}

	void Fault::setFaultType(int row_index, int col_index, int v)
	{
		assert(row_index < row&&col_index < column);
		fault_array[row_index][col_index].type = v;
	}

	Fault::Fault(int row_size, int column_size)
	{
		column = column_size;
		row = row_size;
		fault_array.resize(row_size);
